...
def func(args):
    ...
    selection_criteria = 'meanreverting' if args.selection_criteria is None else args.selection_criteria[0].lower()
    ...
    for gpk, gpdf in df.groupby(['profile', 'month']):
        # calculate average mean and std for sub group, e.g., '01', '05', '09', '00'
        ...
        for idx, row in gpdf.iterrows():
            if Analysis.is_contract_in_deliver_month(row['combo'][0], eval_date):
                ...
                for x in x_std:
                    # selection criteria, use row['mean'], instead of gpmean (mean for each sub group)
                    # row['mean'] is the mean of each single combo, i.e., moving average
                    ...
                    if selection_criteria == 'meanreverting':
                        if row['last_value'] < lower_range:
                            ...
                            vlist = vlist + ['SELL']
                            watchlist[x] = watchlist[x].append(dict(zip(*[tlist, vlist])), ignore_index=True)
                    elif selection_criteria == 'breakout':
    ...
    fftmp = '_'.join([eval_date.strftime('%Y%m%d'), 'watchlist', underlying_config_file.split('.')[0], selection_criteria])
    ...
    for k, v in watchlist.items():
        odir = os.path.join(os.getcwd(), 'watchlist', '_'.join([selection_criteria, 'x_std', str(k)]))
        if not os.path.exists(odir):
            os.makedirs(odir)
